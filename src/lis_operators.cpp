	...
	if(rank==0)
		{
			...
			for(int k=1;k<size;++k)
				{
					for(int i=n_loc+(k-1)*(gn/size);i<n_loc+k*(gn/size);++i)
						{
							int RowSize=sp[i].size();
							MPI_Send(&RowSize,1,MPI_INT,k,0,MPI_COMM_WORLD);
							if(sp[i].size())
								{
									for(j=sp[i].begin();j!=sp[i].end();++j)	
										{
	...
	else
		{
			...
			for(int i=0;i<n_loc;++i)
				{
					int RowSize=0;
					MPI_Recv(&RowSize,1,MPI_INT,0,0,MPI_COMM_WORLD,status);
					if(RowSize)
						{
							int col=0;
							for(int j=0;j<RowSize;++j)
								{
									...
									MPI_Recv(&sp_loc[i][col],1,MPI_DOUBLE,0,0,MPI_COMM_WORLD,status);
								}
						}
				}
