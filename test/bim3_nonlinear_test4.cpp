...
LIS_INT main (LIS_INT argc, char* argv[])
{
  ...
  stream_option stream = {2, "Lis_Solution_NonLinear2.txt"};
  ...
  if (rank == 0)
    {
      ...
      plap.read_mesh (data_dir + "mesh_in_cube100.msh");
      ...
      for (int i = 0; i < nnodes; ++i)
        {
          exactsolution[i] = (1.0 / q) * (pow (0.5, q)
                  - pow ((plap.msh.p (0, i) - 0.5) * (plap.msh.p (0, i) - 0.5)
                       ...
                       + (plap.msh.p (2, i) - 0.5) * (plap.msh.p (2, i) - 0.5), q / 2.0));
      ...
      for (int i = 0; i < vnodes.size (); ++i)
        {
          uold[bnodes[i]] = (1.0 / q) * (pow (0.5, q)
              - pow ((plap.msh.p (0, bnodes[i]) - 0.5) * (plap.msh.p (0, bnodes[i]) - 0.5)
                   + (plap.msh.p (1, bnodes[i]) - 0.5) * (plap.msh.p (1, bnodes[i]) - 0.5)
                   + (plap.msh.p (2, bnodes[i]) - 0.5) * (plap.msh.p (2, bnodes[i]) - 0.5), q / 2.0));
          vnodes[i] = 0.0;
        }
  ...
  status = backtracking_inexact_newton<plaplacian, ForcingType1, LIS>
          (plap, uold, sol, forcing, lis_option, option, stream);
