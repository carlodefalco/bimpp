...
main (int argc, char **argv)
{
  ...
  tmsh.vtk_export ("p4est_dr_test_5_metrics");
  ...
  for (int adapt = 0; adapt < refine_steps; ++adapt)
    {
      ...
      std::vector<double> f(tmsh.num_local_quadrants (), -1.0);
      std::vector<double> g(tmsh.num_global_nodes (), 0.0);

      std::vector<double> fm(tmsh.num_local_quadrants (), -1.0);
      std::vector<double> gm(tmsh.num_global_nodes (), 0.0);
              ...
              if (! quadrant->is_hanging (ii))
                {
                  ...
                  g[quadrant->gt(ii)] = 4*std::cos(std::pow(-0.5 + x,2) + std::pow(-0.5 + y,2)) - 
                    4*std::pow(-0.5 + x,2)*std::sin(std::pow(-0.5 + x,2) + std::pow(-0.5 + y,2)) - 
                    4*std::pow(-0.5 + y,2)*std::sin(std::pow(-0.5 + x,2) + std::pow(-0.5 + y,2));
                  ...
                  if ( std::pow(x - 0.5, 2) +
                       ...
                       std::pow(R, 2))
                    {
                      ...
                      g[quadrant->gt(ii)] = 1.0e11 * std::sin (std::pow (0.25,2));
                      gm[quadrant->gt(ii)] = 1.0e11;
                    }
                }
            }
        }
      ...
      std::vector<double> global_gm(tmsh.num_global_nodes(), 0);
      MPI_Allreduce(gm.data(), global_gm.data(), gm.size(),
                    MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
      
      bim2a_advection_eafe_diffusion (tmsh, global_alpha, psi, A);
      bim2a_reaction (tmsh, fm, global_gm, A);
      ...
      func u_ex =
        ...
        {
          if ((std::pow (x - 0.5, 2) + std::pow (y - 0.5, 2)) > (R*R))
            return std::sin (std::pow (x - 0.5, 2) + std::pow (y - 0.5, 2));
          else
            return std::sin (std::pow (.25, 2));
        };
      ...
      tmsh.octbin_export ((std::string("p4est_dr_test_5_metrics_u_")
      ...
      tmsh.octbin_export ((std::string("p4est_dr_test_5_metrics_uex_")
      ...
      tmsh.octbin_export ((std::string("p4est_dr_test_5_metrics_du0_x_")
                           + std::to_string(adapt)).c_str(), du0.first);
      tmsh.octbin_export ((std::string("p4est_dr_test_5_metrics_du0_y_")
                           + std::to_string(adapt)).c_str(), du0.second);
      
      tmsh.octbin_export ((std::string("p4est_dr_test_5_metrics_du1_x_")
                           + std::to_string(adapt)).c_str(), du1.first);
      tmsh.octbin_export ((std::string("p4est_dr_test_5_metrics_du1_y_")
      ...
      // Refine.
      tmsh.set_metrics_marker (estimator, 1e-3, 4, 1, 1);
      tmsh.metrics_refine (1e5);
      
      tmsh.vtk_export ((std::string("p4est_dr_test_5_metrics_newmesh_")
