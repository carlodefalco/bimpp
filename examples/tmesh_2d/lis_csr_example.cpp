...
main (int argc, char *argv[])
{
  ...
  lis_matrix_create (MPI_COMM_WORLD, &lis_A);
  lis_matrix_set_size (lis_A, nloc, 0);

  /// *****************
  /// solver.set_lhs_structure ()
  /// *****************
  ...
  A.csr_update (value, col_ind, row_ptr, 0);
  for (int ir = 0; ir < size; ++ir)
    {
      if (ir == rank)
        {
  ...
  lis_matrix_set_csr (value.size (), &(row_ptr[is]), &(col_ind[0]), &(value[0]), lis_A);
  
  lis_matrix_assemble (lis_A);
  char filename[255] = "lis_matrix.mm";
  lis_output_matrix (lis_A, LIS_FMT_MM, filename);
