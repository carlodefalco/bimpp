...
main (int argc, char **argv)
{
  ...
  std::vector<double> xold, xvold, xguess;
  xold.assign (2 * n_nodes, 1.0);
  xvold.assign (2 * n_nodes, 1.0);
  xguess.assign (2 * n_nodes, 1.0);
  
  std::vector<double> x (xold);
  std::vector<double> dx (xold);
  dx.assign (2 * n_nodes, 0.0);
  
  auto ixu  = x.begin ();
  auto ixv  = x.begin () + n_nodes;
  auto idx = x.begin ();
  auto ixo = xold.begin ();

  auto rhsfun_u = [&ixu, &ixv, &ixo, &dt] ()
    {return -(*ixu - *(ixo++)) / dt - std::exp (*(ixu++) - *(ixv++)) + 1.0; };

  auto rhsfun_v = [&ixu, &ixv, &ixo, &dt] ()
    {return -(*ixu - *(ixo++)) / dt + std::exp (*(ixu++) - *(ixv++)); };


  double dtinv = 1 / dt;
  auto expudtinv_u = [&ixu, &ixv, &dtinv] ()
    { return  std::exp (*(ixu++) - *(ixv++)) + dtinv; };

  auto expudtinv_v = [&ixu, &ixv, &dtinv] ()
    { return - std::exp (*(ixu++) - *(ixv++)) + dtinv; };
  ...
  int index_base = lin_solver->get_index_base ();
  std::vector<double> xa, xaA;
  std::vector<int> ir, jc, irA, jcA;
  
  sparse_matrix A_block, A;
  A_block.resize (n_nodes);
  A (2 * n_nodes);
  dirichlet_bcs bcs;
  for (int i = 0; i < 4; ++i)
    bcs.push_back (std::make_tuple (0, i, [](double x, double y){return .0;}));
  
  bim2a_structure (tmsh, A_block);
  A_block.aij (xa, ir, jc, index_base);
  for (int ii = 0; ii < 2; ++ii)
    for (int jj = 0; jj < 2; ++jj)
      for (int i = 0; i < n_nodes; ++i)
	  A[ir + ii * n_nodes - index_base][jc + jj * n_nodes - index_base] = 0;
  ...
  A.aij (xaA, irA, jcA, index_base);
  lin_solver->set_lhs_distributed ();
  lin_solver->set_distributed_lhs_structure (A.rows (), irA, jcA);
  lin_solver->analyze ();
