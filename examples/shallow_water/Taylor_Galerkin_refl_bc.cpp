...
TG2_scheme::TG2_scheme(const Q1& sol,
                       ...
                       const ordering& oUx,
                       const ordering& oUy,
                       const ordering& omesh,
                       const Q1& Z,
                       const double& DELTAT)
: sol(sol), sold(sold), soldd(soldd), incr(incr), sol_onehalf(sol_onehalf), ordh(oh), ordUx(oUx), ordUy(oUy), ordmesh(omesh), Z(Z), DELTAT(DELTAT)
{ }
...
TG2_scheme::first_step (tmesh::quadrant_iterator quadrant)
{
  ...
  double h_cell_average = 0., Ux_cell_average = 0., Uy_cell_average = 0., mesh_cell_average = 0.;
  double source_h_cell_average = 0., source_Ux_cell_average = 0., source_Uy_cell_average = 0.;
  for (int ii = 0; ii < 4; ++ii)
  {
    double hdof_c, Uxdof_c, Uydof_c, meshdof_c;
    ...
    if (! quadrant->is_hanging (ii) )
    {
      ...
      meshdof_c = sol [ordmesh (quadrant->gt (ii) )];
    }
    else
    {
      ...
      Uydof_c   = .5 * (sol [ordUy   (quadrant->gparent (0, ii) )] +
                        sol [ordUy   (quadrant->gparent (1, ii) )]);
      meshdof_c = .5 * (sol [ordmesh (quadrant->gparent (0, ii) )] +
                        sol [ordmesh (quadrant->gparent (1, ii) )]);
    }

    h_cell_average    += hdof_c;
    Ux_cell_average   += Uxdof_c;
    Uy_cell_average   += Uydof_c;
    mesh_cell_average += meshdof_c;

    source_h_cell_average  += h_src_formula  (hdof_c, Uxdof_c, Uydof_c);
    source_Ux_cell_average += Ux_src_formula (hdof_c, Uxdof_c, Uydof_c);
    source_Uy_cell_average += Uy_src_formula (hdof_c, Uxdof_c, Uydof_c);
    ...
    fluxx_mesh_node[ii] = mesh_flux_formula_x(hdof_c, Uxdof_c, Uydof_c, meshdof_c);
    fluxy_mesh_node[ii] = mesh_flux_formula_y(hdof_c, Uxdof_c, Uydof_c, meshdof_c);
  ...
  const auto div_Fmesh_x = .5*((fluxx_mesh_node[1]-fluxx_mesh_node[0]) + (fluxx_mesh_node[3]-fluxx_mesh_node[2]));
  const auto div_Fmesh_y = .5*((fluxy_mesh_node[2]-fluxy_mesh_node[0]) + (fluxy_mesh_node[3]-fluxy_mesh_node[1]));
  const auto div_Fmesh_cell = Dy*div_Fmesh_x + Dx*div_Fmesh_y;
  
  sol_onehalf[ordh    (index_quadrant)] = h_cell_average    - dt / 2. * div_Fh_cell    / area + dt / 2. * source_h_cell_average;
  ...
  sol_onehalf[ordmesh (index_quadrant)] = mesh_cell_average - dt / 2. * div_Fmesh_cell / area;
       ...
       quadrant_nei != quadrant->end_neighbor_sweep (); ++quadrant_nei)
  {
    ...
    sol_onehalf[ordmesh (index_quadrant_nei)] += 0.;
  }
...
TG2_scheme::second_step (tmesh::quadrant_iterator quadrant)
{
  ...
  for (int ii = 0; ii < 4; ++ii){
    if (! quadrant->is_hanging (ii)){
      ...
      Uxdof[ii]   = sol [ordUx   (quadrant->gt (ii))];
      Uydof[ii]   = sol [ordUy   (quadrant->gt (ii))];
      meshdof[ii] = sol [ordmesh (quadrant->gt (ii))];
      ...
      Uydof[ii]   = .5 * (sol [ordUy (quadrant->gparent(0,ii))] +
                          sol [ordUy (quadrant->gparent(1,ii))]);
      meshdof[ii] = .5 * (sol [ordmesh (quadrant->gparent(0,ii))] +
                          sol [ordmesh (quadrant->gparent(1,ii))]);
  ...
  F_star_mesh_x = 0., F_star_mesh_y = 0.;
  ...
  const double & mesh_cell = sol_onehalf[ordmesh (index_quadrant)];
  ...
  F_star_mesh_x = mesh_flux_formula_x(h_cell, Ux_cell, Uy_cell, mesh_cell);
  F_star_mesh_y = mesh_flux_formula_y(h_cell, Ux_cell, Uy_cell, mesh_cell);
  ...
  double mesh_min = mesh_cell, mesh_max = mesh_cell;
  for (auto quadrant_nei = quadrant->begin_neighbor_sweep();
       quadrant_nei != quadrant->end_neighbor_sweep (); ++quadrant_nei)
  {
    const auto & index_quadrant_nei = quadrant_nei->get_global_quad_idx (); 
    ...
    const auto & mesh_current_cell = sol_onehalf[ordmesh (index_quadrant_nei)];
    ...
    mesh_min = std::min(mesh_current_cell, mesh_min);
    mesh_max = std::max(mesh_current_cell, mesh_max);
  }
  ...
  grad_cell_mesh = {.5 * ( (meshdof[3] - meshdof[2]) + (meshdof[1] - meshdof[0]) ) / Dx, .5 * ( (meshdof[2] - meshdof[0]) + (meshdof[3] - meshdof[1]) ) / Dy};
  
  // compute flux limiter, grad limiter
  const auto toll = 1e-6;
  double phi_cell_h = 1., phi_cell_Ux = 1., phi_cell_Uy = 1., phi_cell_mesh = 1.;
  for (int ii = 0; ii < 4; ++ii){
    ...
    const auto mesh_vertex = sol[ordmesh  (quadrant->gt (ii))];
...
//    const auto Uy_vertex = Uy_cell + grad_cell_Uy[0]*Dx_v/2 + grad_cell_Uy[1]*Dy_v/2;
    ...
    flux_limiter(mesh_min, mesh_max, mesh_vertex, mesh_cell, toll, phi_cell_mesh);
  ...
  F_star_mesh_x -= grad_cell_mesh[0]*Dx * phi_cell_mesh * vel_rusanov_cell_y;
  F_star_mesh_y -= grad_cell_mesh[1]*Dy * phi_cell_mesh * vel_rusanov_cell_x;
  ...
  for (int ii = 0; ii < 4; ++ii){
    ...
    const auto mesh_ = der_coeffs_x[ii] * F_star_mesh_x + der_coeffs_y[ii] * F_star_mesh_y;
    
    if (! quadrant->is_hanging (ii)){
      incr [ordh  (quadrant->gt (ii))]   += h_;
      incr [ordUx (quadrant->gt (ii))]   += Ux_;
      incr [ordUy (quadrant->gt (ii))]   += Uy_;
      incr [ordmesh (quadrant->gt (ii))] += mesh_;
      ...
      if (boundary_idxx != tmesh::quadrant_t::NOT_ON_BOUNDARY) {
        ...
        F_star_mesh_x_b = mesh_flux_formula_x(h_cell, -Ux_cell, Uy_cell, mesh_cell);
        F_star_mesh_y_b = mesh_flux_formula_y(h_cell, -Ux_cell, Uy_cell, mesh_cell);
        ...
        auto mesh_b = -der_coeffs_x[ii] * F_star_mesh_x_b + der_coeffs_y[ii] * F_star_mesh_y_b;
...
//        auto Uy_b = -der_coeffs_x[ii] * F_star_Uy_x + der_coeffs_y[ii] * F_star_Uy_y;
        ...
        incr [ordmesh (quadrant->gt (ii))] += mesh_b;
        ...
        for (int jj = 0; jj < 4; ++jj){
          if ( quadrant->is_hanging (jj) && (der_coeffs_y[jj]*der_coeffs_y[ii] > 0.) )
          {
            ...
            mesh_b = 2. * der_coeffs_y[jj] * F_star_mesh_y_b;
...
//        auto Uy_b = der_coeffs_x[ii] * F_star_Uy_x - der_coeffs_y[ii] * F_star_Uy_y;
        ...
        for (int jj = 0; jj < 4; ++jj){
          if ( quadrant->is_hanging (jj) && (der_coeffs_x[jj]*der_coeffs_x[ii] > 0.) )
          {
            ...
            mesh_b = 2. * der_coeffs_x[jj] * F_star_mesh_x_b;
...
//        auto Uy_b = -der_coeffs_x[ii] * F_star_Uy_x - der_coeffs_y[ii] * F_star_Uy_y;
      ...
      incr [ordmesh (quadrant->gparent(0,ii))] += mesh_;
      incr [ordmesh (quadrant->gparent(1,ii))] += mesh_;
...
double
TG2_scheme::mesh_flux_formula_x (const double& h, const double& Ux, const double& Uy, const double& mesh)
{ return h > epsilon ? Ux/h*mesh : 0.; }

double
TG2_scheme::mesh_flux_formula_y (const double& h, const double& Ux, const double& Uy, const double& mesh)
{ return h > epsilon ? Uy/h*mesh : 0.; }
